...
private let TEAMINVITECELLREUSEID = "teamInviteCell"
private let TEAMINVITESUBCELLREUSEID = "teamInviteSubCell"

protocol TeamInvitesViewControllerDelegate: AnyObject {
...
class TeamInvitesViewController: UITableViewController {
    ...
    override func viewDidLoad() {
        ...
        tableView.register(TeamInviteCellView.self, forCellReuseIdentifier: TEAMINVITECELLREUSEID)
        tableView.register(TeamInviteSubCellView.self, forCellReuseIdentifier: TEAMINVITESUBCELLREUSEID)
    ...
    func configureCombineListeners() {
        viewModel?.teamInvitesSubject.sink(receiveValue: { result in
            ...
            case .failure(let error):
                self.showMessage(
                    withTitle: "Uh Oh",
                    message: "Unexpected error fetching invites. \(error.localizedDescription)")
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.row == 0 {
            let cell = tableView.dequeueReusableCell(
                withIdentifier: TEAMINVITECELLREUSEID,
                // swiftlint:disable:next force_cast
                for: indexPath) as! TeamInviteCellView
            let invite = teamInvites[indexPath.section].inviteModel
            cell.viewModel = TeamInviteCellVM(
                teamName: invite.team.teamName,
                inviterName: invite.inviteCreator.username)
            return cell
        } else {
            let cell = tableView.dequeueReusableCell(
                withIdentifier: TEAMINVITESUBCELLREUSEID,
                // swiftlint:disable:next force_cast
                for: indexPath) as! TeamInviteSubCellView
