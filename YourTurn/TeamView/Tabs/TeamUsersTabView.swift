...
class TeamUsersTabView: YtViewController {
    ...
    var viewModel: TeamUsersTabVM? {
        didSet {
            ...
            viewModel.teamInvitesSubject.sink { _ in
            ...
            viewModel.teamInvitesSubject.sink { teamInviteResult in
                ...
                case .failure(let error):
                    self.showMessage(
                        withTitle: "Uh Oh",
                        message: "Error encountered fetching outstanding invites. \(error.localizedDescription)")
    ...
    private func loadViewWithTeamInviteButton() {
        ...
        currentUsersButton.anchor(
            top: inviteUsersButton.bottomAnchor,
            left: view.leftAnchor,
            paddingTop: 12,
            paddingLeft: 8,
            width: screenWidth / 2, height: 44)

        view.addSubview(invitesButton)
        invitesButton.anchor(
            top: inviteUsersButton.bottomAnchor,
            left: currentUsersButton.rightAnchor,
        ...
        tableView.anchor(
            top: currentUsersButton.bottomAnchor,
            left: view.leftAnchor,
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if selected == .users, let viewModel = viewModel {
            let cell = tableView.dequeueReusableCell(
                withIdentifier: TabCellIdentifiers.users,
                // swiftlint:disable:next force_cast
                for: indexPath) as! TeamUsersTabViewCell
            cell.viewModel = TeamUsersTabViewCellVM(user: viewModel.users[indexPath.row])
            return cell

        } else if selected == .invites {
            let cell = tableView.dequeueReusableCell(
                withIdentifier: TabCellIdentifiers.invites,
                // swiftlint:disable:next force_cast
                for: indexPath) as! TeamInvitesTabViewCell
